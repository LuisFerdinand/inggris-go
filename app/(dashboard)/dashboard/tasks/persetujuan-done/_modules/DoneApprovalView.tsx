// app/(dashboard)/dashboard/tasks/persetujuan-done/_modules/DoneApprovalView.tsx
"use client";
...
export function DoneApprovalView() {
  const [selectedTaskId, setSelectedTaskId] = useState<string | null>(null);

  const tasksQuery = trpc.taskBoard.list.useQuery({ status: "pending_director_approval" });
    ...
    <div className="flex h-full flex-col gap-4 px-4 pb-6 pt-4 sm:px-6">
      <div>
        <h1 className="flex items-center gap-2 text-[18px] font-extrabold text-slate-800 sm:text-[20px]">
          ...
          Persetujuan Done Direktur
        </h1>
        <p className="mt-0.5 text-[12.5px] text-slate-400">
          Tugas yang diajukan admin untuk persetujuan penyelesaian — hanya super admin yang dapat
          menandai selesai atau mengembalikannya untuk diperbaiki.
        </p>
      </div>
      ...
      {tasksQuery.isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 px-6 py-14 text-center text-[12.5px] text-slate-400">
          Tidak ada tugas yang menunggu persetujuan done.
        </div>
      ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex w-40 shrink-0 items-center gap-1.5">
                    <Avatar size="sm">
                      <AvatarImage src={task.assignee?.image ?? task.creator?.image ?? undefined} />
